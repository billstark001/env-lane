...
import type { EnvFileRef, EnvSource, ResolveEnvOptions, ResolvedEnv } from './types.js';

export function resolveBuildName(options: ResolveEnvOptions, envKey: string, defaultBuild: string): string {
...
function patternToFile(pattern: string, build: string, localBuildName: string, localOverrideFile: string): string {
...
export async function listEnvFiles(options: ResolveEnvOptions = {}): Promise<EnvFileRef[]> {
  ...
  return config.dotenv.order.map((pattern, index) => {
    const fileName = patternToFile(pattern, build, config.dotenv.localBuildName, config.dotenv.localOverrideFile);
...
export async function resolveInjectedEnv(options: ResolveEnvOptions = {}): Promise<ResolvedEnv> {
  ...
  if (missingRequired.length) throw new Error(`Missing required env file(s): ${missingRequired.map(file => file.relativePath).join(', ')}`);
  ...
  for (const file of files) {
    ...
    if (config.selector.forbidInDotenv && Object.prototype.hasOwnProperty.call(parsed, config.selector.envKey)) {
      throw new Error(`${config.selector.envKey} is a selector and must not be stored in dotenv files (${file.relativePath}).`);
    }
    for (const [key, value] of Object.entries(parsed)) {
      values[key] = value;
      sources[key] = { source: 'dotenv', file: file.path, relativeFile: file.relativePath, line: lineForKey(content, key) };
    }
  }
  ...
  sources[config.selector.envKey] = { source: 'selector', shellOverride: Object.prototype.hasOwnProperty.call(process.env, config.selector.envKey) };

  return { rootDir: config.rootDir, target, build, selectorKey: config.selector.envKey, files, values, sources };
}
