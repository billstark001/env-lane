...
export async function findWorkspaceRoot(cwd = process.cwd()): Promise<string> {
  const marker = await findUp(['pnpm-workspace.yaml', 'package.json', '.git'], { cwd, type: 'file' });
...
export function readPnpmWorkspaceGlobs(rootDir: string): string[] {
  ...
  return Array.isArray(doc?.packages) ? doc.packages.filter((item): item is string => typeof item === 'string') : [];
}

export async function loadEnvLaneConfig(options: { cwd?: string; configFile?: string } = {}): Promise<ResolvedEnvLaneConfig> {
